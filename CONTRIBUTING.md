...
- CI is path-filtered: only the projects affected by your changes are tested,
  so builds stay fast.
- Changes to shared packages (`packages/ui-kit`, `packages/flask-core`,
  `packages/shared-js`) trigger downstream tests for all consumers. Keep this
  in mind when modifying shared code -- a small change can have wide impact.
