...
async function loadData() {
  const resp = await fetch("/tech-tree/data/qsvm.json");
...
function showStudyGuide(targetId) {
  ...
  steps.forEach((step, i) => {
    ...
    stepName.textContent = step.id;
    const stepDepth = document.createElement("div");
    stepDepth.className = "step-depth";
    stepDepth.textContent = "d" + step.depth;
    div.appendChild(stepNum);
    div.appendChild(stepName);
    div.appendChild(stepDepth);
...
canvas.addEventListener("touchstart", (e) => {
  ...
  const nodeId = getNodeAtPos(offsetX, offsetY);
  if (nodeId) {
    selectNode(nodeId);
    requestRender();
  } else {
    dragging = true;
    dragStart = { x: offsetX - cam.x, y: offsetY - cam.y };
  }
}, { passive: false });
...
canvas.addEventListener("touchmove", (e) => {
  ...
  if (dragging) {
    ...
    const offsetX = t.clientX - rect.left;
    const offsetY = t.clientY - rect.top;
    cam.x = offsetX - dragStart.x;
    cam.y = offsetY - dragStart.y;
    requestRender();
  }
}, { passive: false });
...
document.getElementById("target-search").addEventListener("input", (e) => {
  ...
  matches.forEach(n => {
    ...
    depthSpan.className = "suggestion-depth";
    depthSpan.textContent = "d" + n.depth;
...
// Self-contained theme management (no external theme-bootstrap.js dependency)
(function initTheme() {
  ...
  window.__setTheme = window.__setTheme || function (t) {
