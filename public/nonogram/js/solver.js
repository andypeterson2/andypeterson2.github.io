...
(function () {
const { state, $,
        elHistSvg, elQuPlaceholder, elClPlaceholder,
        elQuList, elThresholdInput,
       } = App;
...
function renderClassical({ solutions, rows, cols }) {
  ...
  const sz = App.getBestSolSize(rows, cols);
...
function drawEmptyHistogram() {
  ...
  for (const step of [0, 25, 50, 75, 100]) {
    ...
    s += `<line x1="0" y1="${y}" x2="${cW}" y2="${y}" stroke="#999" stroke-width="1"/>`;
    s += `<text x="-4" y="${y}" text-anchor="end" dominant-baseline="middle"
      font-family="Helvetica,Arial,sans-serif" font-size="8" fill="#999">${step}%</text>`;
  }
  for (let i = 0; i < n; i++) {
  ...
  s += `<line x1="0" y1="0" x2="0" y2="${cH}" stroke="#999" stroke-width="1"/>`;
  s += `<line x1="0" y1="${cH}" x2="${cW}" y2="${cH}" stroke="#999" stroke-width="1"/>`;
...
function drawHistogram({ entries, threshold, rows, cols, totalOutcomes }) {
  ...
  for (const step of [0, 25, 50, 75, 100]) {
    ...
    s += `<line x1="0" y1="${y}" x2="${cW}" y2="${y}" stroke="#ddd" stroke-width="1"/>`;
    s += `<text x="-4" y="${y}" text-anchor="end" dominant-baseline="middle"
