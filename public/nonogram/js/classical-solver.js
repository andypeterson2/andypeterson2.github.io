...
/* =============================================================
   ...
   "0"/"1" string ("1" = filled), which is exactly the shape App.renderClassical
...
(function () {
   ...
   *
   * @param {number} len   number of cells in the line
   * @param {number[]} clue block lengths (run-length clue)
   * @returns {number[]} bitmask patterns
   */
  function linePatterns(len, clue) {
    const blocks =
      !clue || clue.length === 0 || (clue.length === 1 && clue[0] === 0) ? [] : clue;
    if (blocks.length === 0) return [0];

    const results = [];
    (function place(blockIdx, start, pattern) {
  ...
  function gridString(chosenRowPats, rows, cols) {
    let s = "";
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        s += (chosenRowPats[r] >> c) & 1 ? "1" : "0";
      }
    }
    return s;
  }
   ...
   *
   * @param {number[][]} rowClues  one run-length clue per row
   * @param {number[][]} colClues  one run-length clue per column
   * @returns {{capped: boolean, solutions: string[]}}
   *   `capped` when the grid exceeds LOCAL_MAX_CELLS (nothing solved);
   *   otherwise every satisfying grid as a row-major "0"/"1" string.
   */
  function solveLocal(rowClues, colClues) {
    ...
    const solutions = [];
    const chosen = new Array(rows);

    (function place(r, colCand) {
      if (r === rows) {
  ...
  // Attach to the shared App namespace (defined by state.js, loaded first).
  window.App = window.App || {};
  App.solveLocal = solveLocal;
  App.LOCAL_MAX_CELLS = LOCAL_MAX_CELLS;
  App.linePatterns = linePatterns; // exposed for tests / debugging
})();
