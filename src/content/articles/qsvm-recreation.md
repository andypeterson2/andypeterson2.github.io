...
The notebook closes with the paper's own noise yardstick — the Jensen–Shannon divergence between ideal and measured output distributions — first under a depolarizing + readout model standing in for the retired IBMQX2, and then **on real hardware**: the same optimized circuit executed on **ibm_marrakesh** (2026, 8192 raw shots) scored **D_JS = 0.0127** against the paper's **0.130** on IBMQX2 in 2019 — *an order of magnitude closer to ideal, before any error mitigation* — the same circuit and the same 16-outcome distribution at 8,192 shots, seven years of hardware later.

One caveat on the yardstick, since the comparison rests on it: the paper defines the KL term of Eq. 33 with a natural log, while this run computes it in bits. Put both in the same units and the gap widens rather than narrows — 0.0127 bits is 0.0088 nats against the paper's 0.130 — so the ratio quoted above is the conservative reading. (The paper also states its divergence lies in [0, 1], the base-2 range, so its own base is ambiguous; either reading leaves the gap at ten times or better.)
