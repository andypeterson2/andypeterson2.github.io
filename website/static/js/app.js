...
(function () {
  ...
  function loadOpenProjectData() {
    var container = document.getElementById("op-projects");
    if (!container) return;

    fetch(API_BASE + "/api/openproject/summary")
      .then(function (r) {
      ...
      .then(function (summaries) {
        if (!summaries.length) {
          container.innerHTML =
            '<div class="empty-state"><p>No OpenProject data available.</p>' +
            '<p class="text-muted">Set OPENPROJECT_TOKEN to enable integration.</p></div>';
          return;
        }

        var html = "";
        summaries.forEach(function (s) {
          var pct = s.sprintProgress || 0;
          var sprintName = s.currentSprint ? s.currentSprint.name : "No sprint";
          var healthClass = pct >= 75 ? "health-green" : pct >= 40 ? "health-yellow" : "health-red";

          html +=
            '<div class="op-project-card">' +
              '<div class="op-project-card-header">' +
                "<h3>" + s.project.name + "</h3>" +
                '<span class="op-sprint-label">' + sprintName + "</span>" +
              "</div>" +
              '<div class="op-progress-row">' +
                '<div class="op-progress-bar">' +
                  '<div class="op-progress-fill" style="width:' + pct + '%"></div>' +
                "</div>" +
                '<span class="op-progress-text">' + pct + "% done</span>" +
              "</div>" +
              '<div class="op-stats">' +
                '<div class="op-stat">' +
                  '<div class="op-stat-value">' + s.totalWorkPackages + "</div>" +
                  '<div class="op-stat-label">Total</div>' +
                "</div>" +
                '<div class="op-stat">' +
                  '<div class="op-stat-value">' + s.inProgressWorkPackages + "</div>" +
                  '<div class="op-stat-label">Active</div>' +
                "</div>" +
                '<div class="op-stat">' +
                  '<div class="op-stat-value ' + healthClass + '">' + s.closedWorkPackages + "</div>" +
                  '<div class="op-stat-label">Closed</div>' +
                "</div>" +
                '<div class="op-stat">' +
                  '<div class="op-stat-value">' + s.sprintClosed + "/" + s.sprintTotal + "</div>" +
                  '<div class="op-stat-label">Sprint</div>' +
                "</div>" +
              "</div>" +
            "</div>";
        });
      ...
      .catch(function (err) {
        container.innerHTML =
          '<div class="empty-state"><p>Failed to load sprint data.</p>' +
          '<p class="text-muted">' + err.message + "</p></div>";
      });
  }

  // Tab switching for OpenProject panel
  var opTabBtn = document.getElementById("tab-btn-openproject");
  var opPanel = document.getElementById("panel-openproject");
  if (opTabBtn && opPanel) {
    opPanel.classList.add("active");
    opTabBtn.addEventListener("click", function () {
      // Deactivate other tabs/panels
      document.querySelectorAll(".tab-btn").forEach(function (b) { b.classList.remove("active"); });
      document.querySelectorAll(".tab-panel").forEach(function (p) { p.classList.remove("active"); });
      opTabBtn.classList.add("active");
      opPanel.classList.add("active");
    });
  }

  // Refresh OpenProject data
  var opRefresh = document.getElementById("op-refresh");
  if (opRefresh) {
    opRefresh.addEventListener("click", loadOpenProjectData);
  }
  ...
  setInterval(loadOpenProjectData, 60000);
})();
