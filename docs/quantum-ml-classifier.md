...
- The weight files (`mnist.json`: 784→10, test accuracy 92.06%; `iris.json`: 4→3, test accuracy 90.0%, plus per-feature ranges for the input form) carry `{kind: "linear", classes, normalize (the exact scale/mean/std the browser must reproduce), weight, test_accuracy, provenance}`. The accuracies are measured on each plugin's real held-out test split — the honest numbers the site surfaces.
