...
|--------|---------|
| `ui-kit/ui-kit.ts` | Shared UI-kit runtime: drawer, dropdown, resize handle, log (also published as the `UIKit` global) |
...
**Tier 1 — live backend.** When a backend is connected (a pass activates it through the gateway; in local dev an allowlisted `?classifiers=` / `?backend=` URL param can point at a localhost server), the app drives the real REST/SSE API below: training with live curves, evaluation, ensembles, ablation, model persistence. On connect it loads the dataset's model types (`/api/datasets/<name>/config`), the server's models and the saved checkpoints. The in-browser models keep answering beside the server's, and a drawing reaches `/predict` as the same cropped, centred 28×28 digit they score. A refused pass (401/402) drops the page back to Tier 2. A stream that fails is reported rather than re-posted: the server keeps running the job, and a second POST would start another and spend pass quota. The `/…/sync` routes are used only when the server has no streaming route (404/405/501).
...
- The weight files come in two kinds, all provenance-stamped. The linear baselines (`mnist.json`: 784→10, test accuracy 92.06%; `iris.json`: 4→3, test accuracy 90.0%, plus per-feature ranges for the input form) carry `{kind: "linear", classes, normalize (the exact scale/mean/std the browser must reproduce), weight, bias, test_accuracy, provenance}`. The QSVM paper recreations (`qsvm-mnist.json`: 6-vs-9, 89.1%; `qsvm-iris.json`: setosa-vs-versicolor, 96.7%; `qsvm-bb84.json`: eavesdropped-vs-clean, 90.8%; each map fitted on a training split and scored on a disjoint held-out one) carry `{kind: "qsvm", classes (the binary pair), w, map: {a,b,c,d}, features, raw_input: "pixels"|"features", ink_threshold (mnist), test_accuracy, train_n, test_n, test_protocol, provenance}` — the paper's solved 2-D rule, no probabilities: the demo's Score column shows its signed margin `s` and its two features instead of a percentage, and the linear models' softmax is labelled as uncalibrated. All accuracies are measured, not asserted.
...
The QSVM paper recreation (Yang et al. 2019, arXiv:1909.11988) lives in the classifier repo as an executed notebook at `notebooks/qsvm-iris/` (own venv — qiskit 2.x wants numpy 2; the service venv is pinned to torch 2.2 / numpy < 2). It reaches the site two ways: the **narrative** is the QSVM section of this project's writeup modal (with the display equations pre-rendered to native MathML — no math JS, CSP-clean), and the **deployable rule** ships as the `qsvm-*.json` weights above, derived closed-form by that repo's `classifiers/qsvm_export.py` (`make export-qsvm`, shipped by `make sync-web`) and drift-checked in its CI. Since 2026-09-04 the rule's quantum α comes from a **real-hardware readout** (the circuit on ibm_marrakesh, 8192 raw shots, D_JS = 0.0127 vs the paper's 0.130 on 2019's IBMQX2 — cached and provenance-stamped as `exports/hardware/` in the classifier repo). `infer.js` dispatches on `kind` and computes the paper's ink-ratio features from the 28×28 canvas for MNIST.
