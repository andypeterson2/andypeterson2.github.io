...
| `PassLane.astro` | Every page, mounted by `BaseLayout`: imports only `src/apps/shared/pass.ts`, which reads a `#pass=` or `?pass=` token, scrubs it from the URL, and installs the fetch wrapper that attaches it to gateway calls. The Cloudflare beacon comes after it, so the token is gone before the beacon runs. |
| `LiveTier.astro` | Demo pages with a backend only, mounted by `DemoShell`: imports the bundled `src/apps/shared/entry.ts` — `pass.ts` first, then `service-config.ts` (URL resolution), `contract-client.ts` (health/discovery client) and `server-connect-modal.ts` (the menubar status item for each backend). |
...
| Global | `<PassLane />` on every page, then the Cloudflare beacon; `DemoShell` adds `<LiveTier />` on demo pages with a backend |
