...
On held-out data the rule gets **96.7% on Iris** (setosa vs versicolor from sepal width and petal length; 29 of 30). The same quantum solution, with only the map values changed, gets **89.1% on MNIST 6-vs-9** (1,000 held-out digits) with the paper's pixel-ratio features. A classical logistic regression fitted to the same points gets 100% and 91.6%, so the quantum rule is a few points under a classical linear model on the same features. The notebook ends with the paper's own noise measure — the Jensen–Shannon divergence between clean and measured output distributions — first under a noise model in place of the retired IBMQX2 computer, then **on real hardware**: the circuit on ibm_marrakesh (2026, 8192 raw shots) scored **0.0127** against the paper's **0.130** on IBMQX2 in 2019 — ten times closer to the clean result. The hardware readout now supplies the deployed rule's quantum values.
